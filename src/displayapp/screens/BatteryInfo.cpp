...
BatteryInfo::BatteryInfo(const Pinetime::Controllers::Battery& batteryController) : batteryController {batteryController} {
  ...
  lv_obj_set_style_radius(charging_bar, LV_RADIUS_CIRCLE, LV_STATE_DEFAULT);
  lv_obj_set_style_bg_color(charging_bar, Colors::bgAlt, LV_STATE_DEFAULT);
  lv_obj_set_style_local_bg_opa(charging_bar, LV_STATE_DEFAULT, LV_STATE_DEFAULT, LV_OPA_100);
  lv_obj_set_style_bg_color(charging_bar, PINETIME_COLOR_RED, LV_BAR_PART_INDIC);
  ...
  lv_obj_set_align(percent, LV_LABEL_ALIGN_LEFT);
...
void BatteryInfo::Refresh() {
  ...
  if (batteryController.IsCharging()) {
    ...
    lv_obj_set_style_bg_color(charging_bar, PINETIME_COLOR_BLUE, LV_BAR_PART_INDIC);
    lv_label_set_text_static(status, "Fully charged");
  } else if (batteryPercent < 10) {
    lv_obj_set_style_bg_color(charging_bar, PINETIME_COLOR_YELLOW, LV_BAR_PART_INDIC);
    lv_label_set_text_static(status, "Battery low");
  } else {
    lv_obj_set_style_bg_color(charging_bar, Colors::highlight, LV_BAR_PART_INDIC);
