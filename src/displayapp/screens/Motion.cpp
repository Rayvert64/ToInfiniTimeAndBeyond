...
Motion::Motion(Controllers::MotionController& motionController) : motionController {motionController} {
  ...
  lv_obj_align(chart, nullptr, LV_ALIGN_TOP_MID, 0, 0);
  ...
  /*Add 3 data series*/
  ser1 = lv_chart_add_series(chart, PINETIME_COLOR_RED);
  ser2 = lv_chart_add_series(chart, Colors::green);
  ser3 = lv_chart_add_series(chart, PINETIME_COLOR_YELLOW);
  ...
  lv_obj_align(label, nullptr, LV_ALIGN_TOP_MID, 0, 10);
  lv_label_set_recolor(label, true);

  labelStep = lv_label_create(lv_scr_act());
  lv_obj_align(labelStep, chart, LV_ALIGN_BOTTOM_LEFT, 0, 0);
