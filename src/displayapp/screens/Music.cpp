...
static void eventHandlerVolUp(lv_event_t* event) {
  Music* screen = static_cast<Music*>(event->user_data);
  screen->VolUp();
}

static void eventHandlerVolDown(lv_event_t* event) {
  Music* screen = static_cast<Music*>(event->user_data);
  screen->VolDown();
}

static void eventHandlerPrev(lv_event_t* event) {
  Music* screen = static_cast<Music*>(event->user_data);
  screen->Prev();
}

static void eventHandlerNext(lv_event_t* event) {
  Music* screen = static_cast<Music*>(event->user_data);
  screen->Next();
}

static void eventHandlerPlayPause(lv_event_t* event) {
  Music* screen = static_cast<Music*>(event->user_data);
  screen->PlayPause();
}
...
bool Music::OnTouchEvent(Pinetime::Applications::TouchEvents event) {
  switch (event) {
    case TouchEvents::SwipeUp: {
      if (lv_obj_has_flag(btnVolDown, LV_OBJ_FLAG_HIDDEN)) {
        lv_obj_remove_flag(btnVolDown, LV_OBJ_FLAG_HIDDEN);
      }
      if (lv_obj_has_flag(btnVolUp, LV_OBJ_FLAG_HIDDEN)) {
    ...
    case TouchEvents::SwipeDown: {
      if (lv_obj_has_flag(btnNext, LV_OBJ_FLAG_HIDDEN)) {
        ...
        if (lv_obj_has_flag(btnPrev, LV_OBJ_FLAG_HIDDEN)) {
