...
SettingSetTime::SettingSetTime(Pinetime::Controllers::DateTime& dateTimeController,
  ...
  lv_obj_t* staticLabel = lv_label_create(lv_scr_act());
  ...
  lv_roller_set_selected(hourCounter.get(), dateTimeController.Hours(), LV_ANIM_ON);
  ...
  lv_roller_set_selected(minuteCounter.get(), dateTimeController.Minutes(), LV_ANIM_ON);
  ...
  lv_obj_add_event_cb(btnSetTime, SetTimeEventHandler);
...
void SettingSetTime::SetTime() {
  const int hoursValue = lv_roller_get_selected(hourCounter.get());
  const int minutesValue = lv_roller_get_selected(minuteCounter.get());
